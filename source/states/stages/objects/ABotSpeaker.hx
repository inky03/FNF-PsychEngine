...
class ABotSpeaker extends FlxSpriteGroup
{
	...
	public function new(x:Float = 0, y:Float = 0)
	{
		...
		eyes.animation.curAnim.curFrame = eyes.anim.curAnim.numFrames - 1;
		add(eyes);
		...
		speaker.animation.curAnim.curFrame = speaker.anim.curAnim.numFrames - 1;
	...
	override function update(elapsed:Float):Void
	{
		...
		{
			//trace(levelMax);
			if(oldLevelMax <= levelMax && (levelMax >= 5 || speaker.animation.curAnim.curFrame >= 3))
				beatHit();
		}
	}
	#end
